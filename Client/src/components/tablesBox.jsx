...
function TablesBox({ qty, capacity, index, handleDelete, handleEdit }) {
  return (
    <div className="py-2 bg-slate-300  rounded-lg w-full flex my-1 p-3 justify-around">
      ...
      <button onClick={() => handleEdit(index)} className=" flex justify-center items-center bg-black bg-opacity-15 rounded-full">
        <PencilIcon className="mx-1 w-[60%] text-blue-600 h-6 cursor-pointer" />
      </button>
      <button onClick={() => handleDelete(index)} className="flex justify-center items-center mx-1 bg-black bg-opacity-15 rounded-full">
